...
public:
    ...
    void AddDataToMEsh(int mesh_index,Eigen::MatrixXd &OV,Eigen::MatrixXi &OF,Eigen::MatrixXd &vertexNormals,const Eigen::MatrixXd textureCoords);

private:
    static void UpdateDataAndBindMesh(igl::opengl::ViewerData& viewerData, const Program& program); // helper function

    static std::vector<igl::opengl::ViewerData> CreateViewerData(const std::shared_ptr<Mesh>& meshData);
