...
and nt_float str = (*(+|-)? (⟨FloatA⟩ | ⟨FloatB⟩ | ⟨FloatC⟩)*)
  ...
  and nt_char_named str = 
    let nt1 = 
      disj_list [pack (word "newline") (fun _ -> '\n');
                 pack (word "return") (fun _ ->  '\r');
                 pack (word "page") (fun _ -> '\012');
                 pack (word "tab") (fun _ -> '\t');
                 pack (word "space") (fun _ -> ' ');
                 pack (word "nul") (fun _ -> '\000')] in
    nt1 str
  ...
  and nt_vector str = 
  (*#( <Sexpr>^* )*)
  let nt1 = make_nt_spaced_out (word "#(") in
  ...
  and nt_list str = 
      let nt1 = make_nt_spaced_out (word "(") in
      let nt2 = pack (char ')') (fun _ -> ScmNil) in
