...
void HighScores::grabCurrentHighScores() {
    ...
    if (fileExists(fileName))
    {
        ...
        for (int i = 0; i < numOfScoresInTable; i++)
        {
            ...
            rowHighScores* row = new rowHighScores();
...
int sortScores(rowHighScores a, rowHighScores b) {
...
void HighScores::saveToHighScores(std::string playerName, int score) {
    if (fileExists(fileName)) {
        ...
        if (!hasPassedLimit) {
            ...
            rowHighScores* newRow = new rowHighScores();
            newRow->score = score;
            ...
            std::vector<rowHighScores> rowsTemp;
            for (int i = 0; i < highscoreRows.size(); i++)
