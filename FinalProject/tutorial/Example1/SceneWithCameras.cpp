...
void SceneWithCameras::BuildImGui()
{
    ...
    ImVec2 start_pos = ImVec2(ImGui::GetIO().DisplaySize.x /4, ImGui::GetIO().DisplaySize.y/4) ;
    ImGui::SetWindowPos(start_pos, ImGuiCond_Always);
    ImVec2 size = ImVec2(ImGui::GetIO().DisplaySize.x/2, ImGui::GetIO().DisplaySize.y/2);
    ImGui::SetWindowSize(size, ImGuiCond_Always);
    if (ImGui::Button("Start the game", ImVec2(400,100)))
...
void SceneWithCameras::startGameMenu(std::vector<igl::opengl::ViewerCore>& core) {
    int imageWidth = 0;
    int imageHeight = 0;
    ...
    name_len  = sizeof(name)/sizeof(char);
    ImGui::Spacing();
    ImGui::Spacing();
    ImGui::Spacing();
    if (game->getLevel() > 0) {
...
void SceneWithCameras::startGameMenuPaused(std::vector<igl::opengl::ViewerCore>& core)
{
    int imageWidth = 0;
    int imageHeight = 0;
    ImGui::SetNextWindowPos(ImVec2(0, 0), ImGuiSetCond_FirstUseEver);
...
void SceneWithCameras::startGameMenuPlay(std::vector<igl::opengl::ViewerCore>& core){
    int imageWidth = 0;
    int imageHeight = 0;
...
void SceneWithCameras::startGameMenuNextLevel(std::vector<igl::opengl::ViewerCore>& core) {
    int imageWidth = 0;
    int imageHeight = 0;
    ...
    if (ImGui::Button("Next level", ImVec2(-1, 0)))
    {
        game->increaseLevel();
        game->increaseMoney();
        highScores->saveToHighScores(name, game->getTotalMoney());
        game->initGame();
        game->setGameState(GameState::PLAY);
    }
    ImGui::Spacing();
...
void SceneWithCameras::startGameMenuWin(std::vector<igl::opengl::ViewerCore>& core) {
    int imageWidth = 0;
    int imageHeight = 0;
...
void SceneWithCameras::startGameMenuLose(std::vector<igl::opengl::ViewerCore>& core) {
    int imageWidth = 0;
    int imageHeight = 0;
...
void SceneWithCameras::startGameStore(std::vector<igl::opengl::ViewerCore>& core) {
    int imageWidth = 0;
    int imageHeight = 0;
