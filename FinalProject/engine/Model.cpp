...
namespace cg3d {
    ...
    std::vector<igl::opengl::ViewerData> Model::CreateViewerData(const std::shared_ptr<Mesh> &mesh) {
        ...
        for (auto &meshData: mesh->data) {
            ...
            viewerData.uniform_colors(Eigen::Vector3d(1.0, 1.0, 1.0), Eigen::Vector3d(1.0, 1.0, 1.0),
                                      Eigen::Vector3d(1.0, 1.0, 1.0)); // todo: implement colors
...
void Model::SetTreeAndCube(std::shared_ptr<Model> _cube, igl::AABB<Eigen::MatrixXd, 3> tree) {
        cube = std::move(_cube);
        treeA1 = std::move(tree);
...
void Model::setMeshData(const std::string& name,  Eigen::MatrixXd _V,  Eigen::MatrixXi _F,Eigen::MatrixXd _VN, Eigen::MatrixXd textureCoords){
